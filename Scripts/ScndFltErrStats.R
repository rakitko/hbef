...
#-------------------------------------------------------------------
# RMSEs in B^a_k for HBEF & EnKF


t1 <- 2000 # 10000  #3000
t2 <- 2400 # 11000 #3200

plot(c(t1:t2), B_a_hbef_RMSE[t1:t2], type="n", xlab="Time", 
     ylab="RMSE(B_a - B_true)", ylim=c(0,15))
lines(c(t1:t2),S_enkf_RMSE  [t1:t2], type="l",col="cadetblue", lwd=2.0)
lines(c(t1:t2),B_a_hbef_RMSE[t1:t2], type="l",col="lightcoral", lwd=2.0)
...
lines(c(t1:t2),B_a_hbef_RMSE[t1:t2], type="l", lwd=2.0, lty=1)

leg.txt<-c("EnKF", "HBEF")
legend("topright", inset=0,leg.txt, lty=c(3,1), lwd=c(2,2), cex=0.8, pt.cex=1, bg="white")
