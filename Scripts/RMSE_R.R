...
# square-root of the observation-error variance R, 
# see Fig.8(top,right) in the paper
#
...
range           <- c(0.25,0.5,1,2,4,8,16,32) # range of sqrt(R)
...
pdf("RMSE_R.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", log="x",ylim=c(0,0.4), ylab="RMSE minus KF-reference-RMSE",xlab="sqrt(R)",xaxt='n')
...
pdf("RMSE_R_bw.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", log="x",ylim=c(0,0.4), ylab="RMSE minus KF-reference-RMSE",xlab="sqrt(R)",xaxt='n')
axis(1, at=range, labels=range)
abline(v=range,h = seq(0,4.5,0.1), col = "gray80", lwd=0.25, lty = 3)
