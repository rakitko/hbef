...
# for the 5 filters (KF, Var, EnKF, HEnKF, and HBEF) -- as functions of the 
# degree of instability of the system measured by the probability pi of the 
#  event |F_k|>1: pi=Pr(|F_k|>1), 
# see Fig.8(bottom,left) in the paper
#
...
range           <- seq(0.01,0.1, by=0.01) # range of pi
...
pdf("RMSE_pi.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", ylim=c(0,0.35), ylab="RMSE minus KF-reference-RMSE",xlab=expression(pi),xaxt='n')
axis(1, at=range, labels=range)
abline(v=range,h = seq(0,4.5,0.05), col = "gray80", lwd=0.25, lty = 3)
...
legend("topleft", inset=0,leg.txt,col=leg.col, lwd=c(3,3,3,3,3), pt.lwd=3, cex=1.1, pt.cex=1, bg="white")
dev.off()


pdf("RMSE_pi_bw.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", ylim=c(0,0.35), ylab="RMSE minus KF-reference-RMSE",xlab=expression(pi),xaxt='n')
axis(1, at=range, labels=range)
abline(v=range,h = seq(0,4.5,0.05), col = "gray80", lwd=0.25, lty = 3)
...
leg.txt<-(c("KF reference",'Var','EnKF', 'HEnKF','HBEF'))
legend("topleft", inset=0,leg.txt, lty=c(1,4,5,6,2), lwd=c(3,2,3,3,4), pt.lwd=c(NA,NA,NA,3,NA), cex=1.0, pt.cex=1, bg="white")
dev.off()
