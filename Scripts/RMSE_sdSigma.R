...
# degree of variability of the system-noise (model error)  measured by the st.dev of Sigma, 
# see Fig.8(bottom,right) in the paper
#
...
range           <- seq(0.1,1, by=0.1) # range of std(Sigma)
...
pdf("RMSE_sdSigma.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", ylim=c(0,1.2), ylab="RMSE minus KF-reference-RMSE",xlab=expression(paste("s.d.", Sigma)),xaxt='n')
...
pdf("RMSE_sdSigma_bw.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", ylim=c(0,1.2), ylab="RMSE minus KF-reference-RMSE",xlab=expression(paste("s.d.", Sigma)),xaxt='n')
axis(1, at=range, labels=range)
abline(v=range,h = seq(0,4.5,0.2), col = "gray80", lwd=0.25, lty = 3)
