...
plot(1, xlim=c(t1,t2), ylim=c(-20,6), type="n", xlab='Time', ylab='X')
...
lines(c(t1:t2),output_hbef$X_a[c(t1:t2)], col="lightcoral",lwd=2)
lines(c(t1:t2),output_kf$X_a[c(t1:t2)],   col="darkblue",lwd=3)

leg.txt<-(c("Truth",'EnKF','HBEF','KF'))
leg.col<-c("black", "cadetblue"," lightcoral ", "darkblue")
legend("topright", inset=0, horiz = TRUE,leg.txt,col=leg.col, lwd=c(1,2,2,3), cex=0.9, pt.cex=1, bg="white")
dev.off()
...
plot(1,xlim=c(t1,t2), ylim=c(-12,5), type="n", xlab='Time', ylab='X', lwd=1)
#abline(v=seq(0, 5000, 50),h = seq(-15,15,5), col = "gray80", lwd=0.25, lty = 2)
points(c(t1:t2),world$X_obs[c(t1:t2)], cex=0.8)

lines(c(t1:t2),world$X[c(t1:t2)],         lty=3,lwd=2) 
lines(c(t1:t2),output_enkf$X_a[c(t1:t2)], lty=1,lwd=1.0)
lines(c(t1:t2),output_hbef$X_a[c(t1:t2)], lty=2,lwd=2)
lines(c(t1:t2),output_kf$X_a[c(t1:t2)],   lty=1,lwd=2)
leg.txt<-(c("Truth",'EnKF','HBEF','KF'))
legend("topright",  inset=0, horiz = TRUE,leg.txt, lty=c(3,1,2,1), lwd=c(2,1,2,2), cex=0.9, pt.cex=1, bg="white")
dev.off()

message("RMS Differences of KF from the truth")
rms(output_kf$X_a[c(1:parameters$time)] - world$X[c(1:parameters$time)])

message("RMS Differences of EnKF and HBEF from the KF")
rms(output_enkf$X_a[c(1:parameters$time)] - output_kf$X_a[c(1:parameters$time)])
rms(output_hbef$X_a[c(1:parameters$time)] - output_kf$X_a[c(1:parameters$time)])
