...
pdf("RMSE_N_NoHEnKF.pdf", width=5.51, height=5.51)
...
leg.txt<-(c("KF reference",'Var','EnKF','HBEF'))
leg.col<-c("darkblue", "navajowhite3","cadetblue","lightcoral")
legend("topright", inset=0,leg.txt,col=leg.col, lwd=c(3,3,3,3), pt.lwd=3, cex=1.1, pt.cex=1, bg="white")
dev.off()


pdf("RMSE_N_NoHEnKF_bw.pdf", width=5.51, height=5.51)
plot(range,rms_kf, type="n", ylim=c(0,0.6), ylab="RMSE minus KF-reference-RMSE",xlab="N",xaxt='n')
axis(1, at=range)
abline(v=range,h = seq(0,4.5,0.1), col = "gray80", lwd=0.25, lty = 3)
...
leg.txt<-(c("KF reference",'Var','EnKF', 'HBEF'))
legend("topright", inset=0,leg.txt, lty=c(1,4,5,2), lwd=c(3,2,3,4), pt.lwd=c(NA,NA,NA,NA), cex=1.0, pt.cex=1, bg="white")
dev.off()
